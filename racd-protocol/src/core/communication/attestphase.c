...
{
    UsefulBuf buf;
    //printf("%s", records->record[0].event.e[0].file_name);
    ...
    //QCBOREncode_AddBytes(&eCtx, (UsefulBufC){records->nonce, crypto_box_NONCEBYTES});
    QCBOREncode_OpenArray(&eCtx);
...
int eventrecords_decode(events *evlist, bool selection, eventrecords *records, uint8_t *buf, uint64_t buf_len)
{
    ...
    {
        // QCBORDecode_GetNext(&DCtx, &item);
        // if (item.uDataType == QCBOR_TYPE_BYTE_STRING && crypto_box_NONCEBYTES == item.val.string.len)
        // {
        //     records->nonce = memcpy(malloc(item.val.string.len), item.val.string.ptr, item.val.string.len);
        // }
        QCBORDecode_GetNext(&DCtx, &item);
...
uint8_t *send_attestresponse(mbedtls_x509_name subject, TPM2B_PUBLIC *public_key, uint8_t *p_buf, uint64_t len, uint32_t *buf_len)
{
    ...
    if (is_authenticated(subjectname, &evlist))
    {
        ...
        //TODO CHANGE AFTER TPM QUTOE int.
        /* --- TPM quote --- */
        ...
        //printf("Preparing TPM quote data.\n");
    ...
    // FILE *f1 = fopen("ppra_key_exec.csv", "a");
    // fprintf(f1, "%lu\n", key_exec);
    // //printf("%lu\n", key_exec);

    // fclose(f1);
    // if((charra_r = tpm2_read_public(esys_ctx,&sig_key_handle,&public_key2)) !=TSS2_RC_SUCCESS){
    //     printf("Could not load pub key from NVRAM.");
    // }
    ...
    // });
    // f1 = fopen("ppra_quote_exec.csv", "a");
	// fprintf(f1, "%lu\n", quote_exec);
	// //printf("%lu\n", quote_exec);
...
void verify_attestresponse(uint8_t nonce[crypto_box_NONCEBYTES], char *rim_path, uint8_t *buf, uint64_t len)
{
    ...
    //printf("records counte in response : %zu\n", records.count);

    //printf("\n\r");
    //printf(" record : %s\n",records.record[6].event.e[0].file_name);

    /* compute PCR composite digest from reference PCRs */
    uint8_t pcr_composite_digest[TPM2_SHA256_DIGEST_SIZE] = {0};

    //uint8_t pcr_composite_digest_mbed[TPM2_SHA256_DIGEST_SIZE] = {0};
    ...
    for (j = 0; j < records.count; j++)
    {
        ...
        if (records.record[j].c != NULL)
        {
            ...
            //printf("%s",verify[index] ? "true" : "false");
            //printf(" record : %s\n", records.record[j].event.e[0].file_name);
            if (!verify[index])
            {
                break;
            }
        ...
        // {
        //     printf("%02x", pcr_folding_digest[records.record[j].pcr][i]);
        // }
        // printf("\n\r");
        //     // for (uint64_t i = 0; i < 32; i++)
        //     // {
        //     //     printf("%02x", records.record[j].event_hash[i]);
        //     // }
        //     // printf("\n\r");
        //     // for (uint64_t i = 0; i < 64; i++)
        //     // {
        //     //     printf("%02x", records.record[j].c[i]);
        //     // }
        //     // printf("\n\r");
        //     // for (uint64_t i = 0; i < 32; i++)
        //     // {
        //     //     printf("%02x", records.record[j].s[i]);
        //     // }
        //     // printf("\n\r");
    }
    //printf("%d,%d,%d,%d\n",index,rim.count,rim_match,rim.count);
    ...
    //printf("%d", memcmp(tpm2_public_key, (TPM2B_PUBLIC *)res.tpm2_public_key, res.tpm2_public_key_len));
    // printf(" Loading TPM key.\n");
    // if ((charra_r = charra_load_external_public_key(esys_ctx, tpm2_public_key,
    //                                                 &sig_key_handle)) != TSS2_RC_SUCCESS)
    // {
    //     printf(" Loading external public key failed.");
    // }
    // else
    // {
    //     printf(" External public key loaded.\n");
    // }

    //printf("Preparing TPM Quote verification.\n");
    ...
    compute_composite_digest_sha256(pcr_composite_digest, pcr_folding_digest, PCR); //TODO dynamic pcr currently hardcoded for 10
    // printf(
    //     " Computed PCR composite digest from reference PCRs is:\n");
    // charra_print_hex(sizeof(pcr_folding_digest[10]), pcr_folding_digest[10],
    //                  "                                   0x", "\n", false);
    // printf(
    //     " Computed PCR composite digest from reference PCRs is:\n");
    ...
    // 		printf(
    // 			" TSS cleanup sig_key_handle failed.");
    // 	}
    // }
...
size_t get_size_of_eventrecords(eventrecords *records)
{
    ...
    // size += get_size_for_cbor_uint(crypto_box_NONCEBYTES);
    ...
    for (uint8_t i = 0; i < records->count; i++)
    {
        ...
        if (records->record[i].c != NULL)
        {
            ...
            //size += crypto_generichash_BYTES_MAX;
...
void simulate_measured_boot(char *filepath) //simulate_measured_boot
{
    ...
    if (access(LOG_FILE_NAME, F_OK) == -1)
    {
...

        {
            ...
            //nizkverify_eventrecord(&records.record[index], NULL);

            //TODO fill TPMLVALUES DIGEST
            ...
            // for (uint64_t i = 0; i < 32; i++)
            // {
            //     printf("%02x", records.record[index].event_hash[i]);
            // }
            // printf("\n\r");
            // for (uint64_t i = 0; i < 64; i++)
            // {
