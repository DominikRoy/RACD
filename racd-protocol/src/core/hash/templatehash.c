...
int templatehash(char* filepath, unsigned char digest[TPM2_SHA256_DIGEST_SIZE])
{
    ...
    /*TODO check if changes in the performance*/
    //unsigned char out[crypto_hash_sha256_BYTES];
    // crypto_hash_sha256_state state;

    // crypto_hash_sha256_init(&state);

    // crypto_hash_sha256_update(&state, temp, TPM2_SHA256_DIGEST_SIZE);
    // crypto_hash_sha256_update(&state, filepath, filepathlength);    
    // crypto_hash_sha256_update(&state, "/", 1);
    // crypto_hash_sha256_update(&state, filename, filenamelength);
