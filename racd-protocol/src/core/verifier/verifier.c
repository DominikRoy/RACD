...
int main(int argc, char *argv[])
{
    ...
    do
    {
        ...
        {
            //printf("close notify %d", data_position);
            clock_gettime(CLOCK_REALTIME, &begin);
            ...
            //uint64_t elapsed = (uint64_t)seconds * 1000 + (uint64_t)nanoseconds / 1e6;
            //double elapsed = seconds + nanoseconds*1e-9;
            double exec = seconds * 1000 + nanoseconds / 1e6;

            FILE *f = fopen("ppra_verifier_50_new.csv", "a");
            fprintf(f, "%.20f\n", exec);
            fclose(f);
            break;
        }
        ...
        counter++;
        //verify_attestresponse(nonce,swSelection_file,buf, len);
...
exit:
    ...
    // FILE *f = fopen("ppra_ver_exec_mixed_remote_load_mem_docker.csv", "a");
    // fprintf(f, "%.20f\n", exec);
    // fclose(f);
